  ...
  return (
    <>
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 md:static transition-colors duration-300 ${headerWhite ? "bg-white shadow-sm" : "bg-transparent"}`}>
        ...
        {/* Mobile menu — tela cheia */}
        <div className={`md:hidden fixed top-0 left-0 right-0 bottom-0 z-40 bg-white transition-all duration-300 ${menuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
          <div className="flex justify-between items-center px-6 py-4">
            ...
            <button className="flex flex-col justify-center items-center gap-[5px] w-8 h-8 cursor-pointer" onClick={() => setMenuOpen(false)} aria-label="Fechar menu">
              <span className="block w-6 h-[1.5px] bg-[#17445B] rotate-45 translate-y-[1px]" />
              <span className="block w-6 h-[1.5px] bg-[#17445B] -rotate-45 -translate-y-[1px]" />
            </button>
          </div>
          ...
          <nav className="flex flex-col items-center justify-center gap-10 h-[calc(100%-76px)]">
            {navLinks.map((item) => (
              <a key={item.href} href={item.href} onClick={() => setMenuOpen(false)} className="font-[Tenor_Sans] text-[18px] text-black tracking-widest hover:opacity-60 transition-opacity">
                {item.label}
              </a>
            ))}
          </nav>
