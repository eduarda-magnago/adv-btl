...
export default function Header() {
  ...
  return (
    <>
      <div className="block md:hidden h-[88px]" />

      <header className={`fixed top-0 left-0 right-0 z-50 md:static transition-colors duration-300 ${headerWhite ? "bg-white shadow-sm md:bg-transparent md:shadow-none" : "bg-transparent"}`}>
        ...
        <div className={`md:hidden fixed top-0 left-0 right-0 bottom-0 z-40 bg-white transition-opacity duration-300 ${menuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
          ...
          <nav className="flex flex-col items-center justify-center gap-10 h-[calc(100%-76px)]">
            {navLinks.map((item) => (
              <a key={item.href} href={item.href} onClick={() => setMenuOpen(false)} className="font-[Tenor_Sans] text-[18px] text-black tracking-widest hover:text-[#53707F] transition-colors">
                {item.label}
              </a>
            ))}
          </nav>
